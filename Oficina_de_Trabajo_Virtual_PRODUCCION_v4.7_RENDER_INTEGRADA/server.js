...
async function route(req,res){
 ...
 if(req.method==='GET'&&(p==='/health'||p==='/health/'))return json(res,{ok:true,service:'oficina-trabajo-virtual',version:'4.5.0',env:PROD?'production':'development',time:now()});
 if(req.method==='GET'&&p==='/ready')return json(res,{ready:true,service:'oficina-trabajo-virtual',version:'4.5.0',database:'ok',time:now()});
 if(req.method==='GET'&&p==='/_render')return json(res,{ok:true,service:'oficina-trabajo-virtual',version:'4.5.0',port:PORT,host:HOST,node:process.version,production:PROD,time:now()});
 ...
 if(req.method==='POST'&&p==='/api/admin/setup'){
  ...
  try{
   ...
   return json(res,{ok:true,message:'Administrador principal creado. Ya puedes iniciar sesión.'});
...
server.listen(PORT,HOST,()=>console.log(`OTV 4.5.0 escuchando en http://${HOST}:${PORT} (Render PORT=${PORT})`));
