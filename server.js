...
function sameOrigin(req){
 ...
 const own=`https://${req.headers.host}`;
 if(origin===own)return true;
 const allowed=String(process.env.FRONTEND_ORIGINS||'').split(',').map(x=>x.trim()).filter(Boolean);
 if(allowed.includes(origin))return true;
 // Allow Netlify-hosted frontends when explicitly opted in. This keeps
 // the Render API usable behind a Netlify reverse proxy.
 try{
   const uo=new URL(origin);
   if(process.env.ALLOW_NETLIFY_ORIGIN==='true' && uo.protocol==='https:' && uo.hostname.endsWith('.netlify.app'))return true;
 }catch{}
 return false;
}
